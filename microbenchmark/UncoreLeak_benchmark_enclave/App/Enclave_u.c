...
static sgx_status_t SGX_CDECL Enclave_ocall_wait_for_user_input(void* pms)
{
...
static sgx_status_t SGX_CDECL Enclave_ocall_idle_seconds(void* pms)
{
	ms_ocall_idle_seconds_t* ms = SGX_CAST(ms_ocall_idle_seconds_t*, pms);
	ocall_idle_seconds(ms->ms_seconds);
...
static const struct {
	...
	14,
	{
		...
		(void*)Enclave_ocall_wait_for_user_input,
		(void*)Enclave_ocall_idle_seconds,
...
sgx_status_t ecall_with_idle_then_work(sgx_enclave_id_t eid)
{
	sgx_status_t status;
